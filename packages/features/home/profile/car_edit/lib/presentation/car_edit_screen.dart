import 'dart:ui';
import 'widgets/controller/car_edit_steps_controller.dart';
...
class _CarEditScreenState extends State<CarEditScreen> {
  late CarEditStepsController controller;
  int _lastIndex = 0;
  @override
  void initState() {
    // controller = Get.put(SellStepsController());

    controller = Get.put(CarEditStepsController(), permanent: false);
    getIt.resetLazySingleton<CarEditCubit>();
  ...
  void dispose() {
    // controller.dispose();
    if (Get.isRegistered<CarEditStepsController>()) {
      Get.delete<CarEditStepsController>();
    }
    getIt<CarEditCubit>().close();
